...
func AcceptJoinRequest(c echo.Context, conn *pgxpool.Pool) error {
	...
	err := conn.QueryRow(
		...
		`SELECT
					...
					FROM join_requests
					JOIN teams
						ON join_requests.team_id = teams.id
					WHERE join_requests.id = $1`,
		requestID,
	).Scan(&teamID, &requesterID, &status, &ownerID)
	...
	if ownerID != userID {
		return c.JSON(http.StatusForbidden, map[string]string{
			"error": "Only team owner can accept join requests",
		})
	}
...
func RejectJoinRequest(c echo.Context, conn *pgxpool.Pool) error {
	...
	if ownerID != userID {
		return c.JSON(http.StatusForbidden, map[string]string{
			"error": "Only team owner can reject join requests",
		})
	}
