...
import { createTeam, uploadTeamCover } from '../services/api'
...
export default function CreateTeam() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (cover) await uploadTeamCover(createdTeamId, cover)
      navigate(`/teams/${createdTeamId}`)
    } catch (err) {
      if (createdTeamId) {
        navigate(`/teams/${createdTeamId}`, {
          state: { teamImageError: err.message },
        })
      } else {
